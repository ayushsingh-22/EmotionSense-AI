...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    } else if (user?.id && !currentSessionId) {
      // Start new session if no session ID in URL and no current session
      startNewSession();
    }
  }, [searchParams, user?.id, continueSession, startNewSession, currentSessionId]);
  ...
  const handleSendMessage = useCallback(async (input: string) => {
    ...
    setMessages((prev) => {
      const base = editing
        ? prev.filter((m) => m.id && m.id !== editing.messageId && m.id !== editing.assistantMessageId)
        : prev;
    ...
    try {
      ...
      const aiMessage: ExtendedChatMessage = {
        id: payload.aiResponse.id || `ai-${Date.now()}`,
        user_id: user.id,
        session_id: newSessionId,
        role: 'assistant',
        content: payload.aiResponse.content || payload.aiResponse.message || '',
        message: payload.aiResponse.message || payload.aiResponse.content || '',
      ...
      setMessages((prev) => {
        const withoutTemp = prev.filter((m) => m.id && !m.id.startsWith('temp-'));
        const base = editing
          ? withoutTemp.filter((m) => m.id && m.id !== editing.messageId && m.id !== editing.assistantMessageId)
          : withoutTemp;
