...
function generateJournalText(insights, date) {
  ...
  const templates = {
    joy: [`Today was filled with joy! ${emotionEmoji} You experienced ${emotionCount} moments of happiness.`, 
           `A wonderful day! ${emotionEmoji} Joy was your dominant emotion with ${emotionCount} positive moments.`],
    sadness: [`Today brought some sadness. ${emotionEmoji} You processed ${emotionCount} moments of melancholy.`,
          `A reflective day. ${emotionEmoji} Sadness emerged ${emotionCount} times as you navigated your emotions.`],
    anger: [`Today had some challenging moments. ${emotionEmoji} Anger surfaced ${emotionCount} times.`,
            `An intense day. ${emotionEmoji} You felt angry in ${emotionCount} instances.`],
    neutral: [`A balanced day. ${emotionEmoji} You maintained emotional equilibrium with ${emotionCount} neutral moments.`,
              `A steady day. ${emotionEmoji} Your emotions stayed relatively neutral throughout.`],
    fear: [`Today brought some anxiety. ${emotionEmoji} Fear or worry appeared ${emotionCount} times.`,
           `A cautious day. ${emotionEmoji} You felt apprehensive or fearful in ${emotionCount} moments.`],
    disgust: [`Today had some challenging moments. ${emotionEmoji} Disgust appeared ${emotionCount} times.`,
              `A difficult day. ${emotionEmoji} You experienced disgust in ${emotionCount} instances.`],
    surprise: [`Today brought unexpected moments! ${emotionEmoji} Surprise emerged ${emotionCount} times.`,
  ...
  const otherEmotions = emotions.filter(e => e !== mainEmotion && emotionDistribution[e] > 0);
  if (otherEmotions.length > 0) {
    const otherEmotionsText = otherEmotions.map(e => 
      `${e} (${emotionDistribution[e]})`).join(', ');
...
function generateFallbackReflection(weekData) {
  const { dominant_emotion, active_days, avg_mood_score, emotion_summary, total_activities } = weekData;
  ...
  const moodLevel = avg_mood_score >= 70 ? 'positive' : avg_mood_score >= 40 ? 'balanced' : 'challenging';
  
  const templates = {
    joy: [`${emotionEmoji} This was a joyful week! You experienced happiness across ${active_days} days.`,
          `${emotionEmoji} A wonderful week filled with positive moments over ${active_days} active days.`],
    sadness: [`${emotionEmoji} This week brought some reflective moments across ${active_days} days.`,
              `${emotionEmoji} A thoughtful week with emotional depth over ${active_days} days.`],
    anger: [`${emotionEmoji} This week had some intense moments spread across ${active_days} days.`,
            `${emotionEmoji} A challenging week with strong emotions over ${active_days} active days.`],
    neutral: [`${emotionEmoji} A balanced week with steady emotions across ${active_days} days.`,
              `${emotionEmoji} This week maintained emotional equilibrium over ${active_days} days.`],
    fear: [`${emotionEmoji} This week brought some anxious moments across ${active_days} days.`,
           `${emotionEmoji} A cautious week with thoughtful reflection over ${active_days} days.`],
    disgust: [`${emotionEmoji} This week had some challenging experiences across ${active_days} days.`,
              `${emotionEmoji} A difficult week that you navigated over ${active_days} active days.`],
    surprise: [`${emotionEmoji} This week brought unexpected moments across ${active_days} days!`,
  ...
  const baseReflection = emotionTemplates[Math.floor(Math.random() * emotionTemplates.length)];
  ...
  if (moodLevel === 'positive') {
    ...
    moodContext = ' Take time to care for yourself and celebrate small victories.';
  } else {
...
function generateTimeSegments(activities) {
  const segments = {
    morning: { period: 'morning', emotion: 'neutral', count: 0 },
    afternoon: { period: 'afternoon', emotion: 'neutral', count: 0 },
    evening: { period: 'evening', emotion: 'neutral', count: 0 }
  };
  ...
  activities.forEach(activity => {
    ...
    // Set the dominant emotion for the period (simple approach - most recent)
    if (segments[period].count === 1 || Math.random() > 0.5) {
      segments[period].emotion = emotion;
    }
  });
...
async function generateWeeklyReflection(weekData) {
  try {
    const { dominant_emotion, avg_mood_score, emotion_summary, active_days, total_activities } = weekData;
...
Keep it under 200 characters. Be specific to the emotions mentioned. Sound natural and supportive.`;
    ...
    const llmPromise = llmService.generateResponse(prompt, 'neutral', [])
      .then(response => {
    ...
    const reflectionText = typeof response === 'string' ? response : fallbackReflection;
    ...
    return `This week showed ${dominant_emotion} as your primary emotion across ${active_days} active days. ${avg_mood_score >= 60 ? 'Overall, it was a positive week.' : 'Take time to care for yourself.'}`;
  }
}
...
router.get('/daily', async (req, res) => {
  try {
    ...
    const end = endDate || DateTime.now().setZone('Asia/Kolkata').toFormat('yyyy-MM-dd');
    const start = startDate || DateTime.now().setZone('Asia/Kolkata').minus({ days: 30 }).toFormat('yyyy-MM-dd');
    ...
    const dailyInsights = await Promise.all(
      Object.keys(dailyMap).map(async (date) => {
        try {
          // Get emotion insights for this date
          const insights = await masterActivityService.getEmotionInsights(userId, date, date);
          
          // Generate journal text based on emotion data
          const journalText = generateJournalText(insights, date);
          
          // Generate time segments from activities
          const timeSegments = generateTimeSegments(dailyMap[date] || []);
          
          // Calculate mood score - ALWAYS prioritize journal entry for consistency
          let moodScore;
          const journalEntry = journalEntries?.find(j => j.date === date);
          if (journalEntry?.emotion_summary?.mood_score) {
            moodScore = journalEntry.emotion_summary.mood_score;
          } else {
            // Fallback: Calculate mood score using unified service
            const activities = dailyMap[date] || [];
            const emotionList = activities.map(a => ({
              emotion: a.emotion_data?.emotion || a.primary_emotion,
              confidence: a.emotion_data?.confidence || a.emotion_data?.emotion_confidence || a.confidence
            })).filter(e => e.emotion);
          ...
          return {
            id: `daily-${userId}-${date}`, // Generate a consistent ID
            user_id: userId,
            date,
            content: journalText, // Formatted journal text
            emotion: insights.dominantEmotion || 'neutral', // Top-level dominant emotion
            emotion_emoji: getEmotionEmoji(insights.dominantEmotion || 'neutral'), // Top-level emoji
            emotion_summary: {
              dominant_emotion: insights.dominantEmotion || 'neutral',
              mood_score: moodScore, // 0-100 scale
              emotion_counts: insights.emotionDistribution || {},
              time_segments: timeSegments,
              context_summary: `Analyzed ${insights.activityCount || 0} activities`, // Brief context
              message_count: insights.activityCount || 0
            },
          ...
          // Fallback journal text
          const journalText = generateFallbackJournalText(emotions, dominantEmotion, dayActivities.length);
          
          // Fallback time segments
          const timeSegments = generateTimeSegments(dayActivities);
          
          // Calculate fallback mood score - ALWAYS prioritize journal entry for consistency
          let moodScore;
          const journalEntry = journalEntries?.find(j => j.date === date);
          if (journalEntry?.emotion_summary?.mood_score) {
            moodScore = journalEntry.emotion_summary.mood_score;
          } else {
            // Fallback: Calculate mood score from activities
            const emotionList = dayActivities.map(a => ({
              emotion: a.emotion_data?.emotion || a.primary_emotion,
              confidence: a.emotion_data?.confidence || a.emotion_data?.emotion_confidence || a.confidence
            })).filter(e => e.emotion);
          ...
          return {
            ...
            date,
            content: journalText, // Formatted journal text
            emotion: dominantEmotion, // Top-level dominant emotion
            emotion_emoji: getEmotionEmoji(dominantEmotion), // Top-level emoji
            emotion_summary: {
              dominant_emotion: dominantEmotion,
              mood_score: moodScore, // 0-100 scale
              emotion_counts: emotions,
              time_segments: timeSegments,
              context_summary: `Processed ${dayActivities.length} activities`, // Brief context
      ...
      try {
        ...
        weekActivities.forEach(activity => {
          if (activity.emotion_data?.emotion) {
            ...
            const activityDate = DateTime.fromISO(activity.created_at).toFormat('yyyy-MM-dd');
        ...
        for (let day = 0; day < 7; day++) {
          ...
          if (dayActivities.length > 0) {
            ...
            if (journalEntry?.emotion_summary?.mood_score) {
              ...
              dominantDayEmotion = journalEntry.emotion || journalEntry.emotion_summary?.dominant_emotion || 'neutral';
            } else {
              // Fallback: Calculate from activities using unified service
              const emotionList = dayActivities.map(a => ({
                emotion: a.emotion_data?.emotion,
                confidence: a.emotion_data?.confidence || a.emotion_data?.emotion_confidence
              })).filter(e => e.emotion);

              avgDayMoodScore = unifiedEmotion.calculateAverageMoodScore(emotionList);
              dominantDayEmotion = unifiedEmotion.getDominantEmotion(emotionList);
            }
        ...
        const weeklyEmotionList = weekActivities.map(a => ({
            emotion: a.emotion_data?.emotion,
            confidence: a.emotion_data?.confidence || a.emotion_data?.emotion_confidence
        })).filter(e => e.emotion);

        const avgMoodScore = unifiedEmotion.calculateAverageMoodScore(weeklyEmotionList);
        ...
        const weekSummary = {
          ...
          daily_arc: dailyArc,
          total_messages: weekActivities.filter(a => a.activity_type === 'chat_message').length,
          total_activities: weekActivities.length,
          // Additional insights
          peak_mood_day: dailyArc.filter(day => day.has_data && day.mood_score !== null).reduce((max, day) => day.mood_score > (max.mood_score || 0) ? day : max, { mood_score: null, emotion: 'neutral', date: '', has_data: false }),
          low_mood_day: dailyArc.filter(day => day.has_data && day.mood_score !== null).reduce((min, day) => day.mood_score < (min.mood_score || 100) ? day : min, { mood_score: null, emotion: 'neutral', date: '', has_data: false }),
        ...
        try {
          ...
          logger.warn(`Failed to generate reflection for week ${i}:`, reflectionError.message);
          weekSummary.reflection_text = generateFallbackReflection(weekSummary);
        }
        
        weeklyInsights.push(weekSummary);
      } catch (error) {
        logger.warn(`Failed to get weekly summary for week ${i}:`, error.message);
        // Add empty week data
        const emptyDailyArc = [];
        const weekStart = today.minus({ weeks: i }).startOf('week');
        ...
        weeklyInsights.push({
          ...
          emotion_emoji: '😐',
          avg_mood_score: 50,
          reflection_text: "This was a quiet week with minimal emotional activity recorded.",
          emotion_summary: {},
          daily_arc: emptyDailyArc,
          total_messages: 0,
          total_activities: 0,
          peak_mood_day: { date: '', mood_score: null, emotion: 'neutral', has_data: false },
          low_mood_day: { date: '', mood_score: null, emotion: 'neutral', has_data: false },
          mood_variance: 0,
          active_days: 0
        });
      }
  ...
  try {
    ...
    const timeline = dayActivities
      ...
      .map(activity => ({
        ...
        content_preview: activity.activity_type === 'chat_message' 
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const endDate = DateTime.now().setZone('Asia/Kolkata').toFormat('yyyy-MM-dd');
    const startDate = DateTime.now().setZone('Asia/Kolkata').minus({ days: 30 }).toFormat('yyyy-MM-dd');
    ...
    for (let i = 0; i < sortedDates.length; i++) {
      ...
      const expectedDate = DateTime.now().setZone('Asia/Kolkata').minus({ days: i });
...
router.get('/moments', async (req, res) => {
  try {
    ...
    const end = endDate || DateTime.now().setZone('Asia/Kolkata').toFormat('yyyy-MM-dd');
    const start = startDate || DateTime.now().setZone('Asia/Kolkata').minus({ days: 7 }).toFormat('yyyy-MM-dd');
    ...
    const keyMoments = activities
      ...
      .map(activity => ({
        ...
        timestamp: activity.created_at,
        context: activity.activity_type === 'chat_message' 
